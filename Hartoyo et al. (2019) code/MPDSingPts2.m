function [z,lambda,Jac] = MPDSingPts2(param,flg)
%
%   [z,lambda,Jac] = MPDSingPts2(param,flg)
%
...
%   
%   param as defined in MPDParam2.m
...
if flg ~= 0
    ...
    function s = Se1(h)
        eh = exp(-r2*(h - param.mu_e)/param.sigma_e);
        if ~isinf(eh)
            s = (r2*param.Se_max*eh/param.sigma_e)/((1 + eh)^2);        
        else
            s = 0;
        end
    end
    function s = Si1(h)
        ih = exp(-r2*(h - param.mu_i)/param.sigma_i);
        if ~isinf(ih)
            s = (r2*param.Si_max*ih/param.sigma_i)/((1 + ih)^2);
        else
            s = 0;
        end
    end
    ...
    function s = Jacobian(x)
        ...
        s(6,1) = param.Gamma_e*param.gamma_ei*exp(1)*param.Nei_beta*Se1(x(1));
        s(6,6) = -2*param.gamma_ei;
        s(6,5) = -param.gamma_ei^2;
        
        s(7,8) = 1;
        
        s(8,2) = param.Gamma_i*param.gamma_ie*exp(1)*param.Nie_beta*Si1(x(2));
        s(8,8) = -2*param.gamma_ie;
        s(8,7) = - param.gamma_ie^2;
    ...
    function s = CheckSingPts(z1)
        s = zeros(size(z1));
        s(1) = param.he_rest - z(1,:) + Psi_ee(z(1,:)).*z(3,:) + Psi_ie(z(1,:)).*z(7,:);
        s(2) = param.hi_rest - z(2,:) + Psi_ei(z(2,:)).*z(5,:) + Psi_ii(z(2,:)).*z(9,:);
        s(4) = -param.gamma_ee^2*z(3,:) + param.Gamma_e*param.gamma_ee*exp(1)*(param.Nee_beta*Se(z(1,:)) + param.pee);
        s(6) = -param.gamma_ei^2*z(5,:) + param.Gamma_e*param.gamma_ei*exp(1)*(param.Nei_beta*Se(z(1,:)) + param.pei);
        s(8) = -param.gamma_ie^2*z(7,:) + param.Gamma_i*param.gamma_ie*exp(1)*(param.Nie_beta*Si(z(2,:)) + param.pie);
        s(10) = -param.gamma_ii^2*z(9,:) + param.Gamma_i*param.gamma_ii*exp(1)*(param.Nii_beta*Si(z(2,:)) + param.pii);
    end
end
