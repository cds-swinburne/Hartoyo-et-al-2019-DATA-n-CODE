function [d2C,le,la] = FisherSeqBase(th,freq,param,dev,psel,fig,mparam)
%
%   [d2C,le,la] = FisherSeqBase(th,freq,param,dev,psel,fig,mparam)
%
...
for n = (1:Ns)
    thn = th(:,n);
    d2C{n} = FisherBase(thn,freq,param,dev,psel,mparam);
    if ~isempty(d2C{n})
        [ev,lambda] = eig(d2C{n});
        lm = diag(lambda);
        Lambda = lm;%abs(lm);
        ...
        for k = (1:Np)
            ...
            if (Lambda(k) > 0)&&(ne < mparam.Ne)
                semilogy([n n+1]-0.5,Lambda(k)*[1 1],mparam.clr,'LineWidth',1);  
                if h == 0
                    hold on
                    h = 1;
                end
